...
class FleetManager {
  public:
    explicit FleetManager(
        std::unique_ptr<IStorage> storage, std::unique_ptr<IPrinter> printer = nullptr,
    ...
    auto updateVehicleStatus(uint32_t vehicle_id, VehicleStatus status,
    ...
    auto searchFleet(Predicate pred, const std::optional<AdvancedFilter>& adv_filter =
                                         std::nullopt) const -> std::vector<Vehicle> {
    ...
    auto rentVehicle(uint32_t vehicle_id, const std::string& name = "",
                     const std::string& surname = "", const std::string& id_card = "",
                     std::string* out_print_warning = nullptr) -> std::optional<std::string>;
    auto returnVehicle(uint32_t vehicle_id, int* out_cost = nullptr, const std::string& notes = "",
                       std::string* out_print_warning = nullptr) -> bool;
    auto returnVehicle(const std::string& rental_code, int* out_cost = nullptr,
                       const std::string& notes = "",
                       std::string* out_print_warning = nullptr) -> bool;
    [[nodiscard]] auto
    getVehicleIdByRentalCode(const std::string& rental_code) const -> std::optional<uint32_t>;
    [[nodiscard]] auto
    getRentalCodeByVehicleId(uint32_t vehicle_id) const -> std::optional<std::string>;
    [[nodiscard]] auto getRentalBilledHours(uint32_t vehicle_id) const -> std::optional<uint32_t>;
    [[nodiscard]] auto idExists(uint32_t vehicle_id) const -> bool;
