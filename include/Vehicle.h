...
struct AdvancedFilter {
    ...
    [[nodiscard]] auto matches(const Vehicle& vehicle, const std::optional<std::string>& rental_code_val) const -> bool {
        return matchesId(vehicle) &&
               matchesBrand(vehicle) &&
               matchesModel(vehicle) &&
               matchesType(vehicle) &&
               matchesStatus(vehicle) &&
               matchesPrice(vehicle) &&
               matchesSeats(vehicle) &&
               matchesPayload(vehicle) &&
               matchesRentalCode(rental_code_val);
    }

private:
    static auto safe_stoul(const std::string& str_val) -> uint32_t {
        try {
    ...
    static auto safe_stod_grosz(const std::string& str_val) -> int {
        try {
    ...
    [[nodiscard]] auto matchesId(const Vehicle& vehicle) const -> bool {
        if (!id.active()) {
            return true;
        }
    ...
    [[nodiscard]] auto matchesBrand(const Vehicle& vehicle) const -> bool {
        if (!brand.active()) {
            return true;
        }
    ...
    [[nodiscard]] auto matchesModel(const Vehicle& vehicle) const -> bool {
        if (!model.active()) {
            return true;
        }
    ...
    [[nodiscard]] auto matchesType(const Vehicle& vehicle) const -> bool {
        if (!type.active()) {
            return true;
        }
    ...
    [[nodiscard]] auto matchesStatus(const Vehicle& vehicle) const -> bool {
        if (!status.active()) {
            return true;
        }
    ...
    [[nodiscard]] auto matchesPrice(const Vehicle& vehicle) const -> bool {
        if (!price.active()) {
            return true;
        }
    ...
    [[nodiscard]] auto matchesSeats(const Vehicle& vehicle) const -> bool {
        if (!seats.active()) {
            return true;
        }
    ...
    [[nodiscard]] auto matchesPayload(const Vehicle& vehicle) const -> bool {
        if (!payload.active()) {
            return true;
        }
        const auto& variant = vehicle.getVariant();
    ...
    [[nodiscard]] auto matchesRentalCode(const std::optional<std::string>& rental_code_val) const -> bool {
